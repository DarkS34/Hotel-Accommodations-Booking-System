...
const postAccommodation = async (req, res, next) => {
  try {
    ...
    if (req.files) {
      newAccommodation.imgs = req.files.map((file) => file.path);
    }
...
const updateAccommodation = async (req, res, next) => {
  try {
    ...
    if (req.files) {
      ...
      newAccommodation.imgs = [...oldAccommodation.imgs, ...newImgs];
    }
...
const deleteAccommodation = async (req, res, next) => {
  try {
    ...
    accommodationDeleted.imgs.forEach((img) => deleteFile(img));
